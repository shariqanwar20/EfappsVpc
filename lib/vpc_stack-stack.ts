  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const vpc = new ec2.Vpc(this, "ControlTowerVpc", {
      ...
      subnetConfiguration: [
        {
          cidrMask: 24,
          name: `${this.account}-${this.region}-PublicSubnet`,
          subnetType: ec2.SubnetType.PUBLIC,
        },
        {
          cidrMask: 24,
          name: `${this.account}-${this.region}-PrivateSubnet`,
          subnetType: ec2.SubnetType.PRIVATE_WITH_NAT,
        },
        {
          cidrMask: 24,
          name: `${this.account}-${this.region}-ServicesSubnet`,
          subnetType: ec2.SubnetType.PRIVATE_WITH_NAT,
        },
      ],
    });
    ...
    cdk.Tags.of(sgBastion).add(
      "Name",
      `${this.account}-${this.region}-sg-bastion`
    );
    ...
    sgPostgress.addIngressRule(
      sgBastion,
      ec2.Port.tcp(5432),
      "SecurityGroupPostgresBastionIngress"
    );
    cdk.Tags.of(sgPostgress).add(
      "Name",
      `${this.account}-${this.region}-sg-postgress`
    );
    ...
    sgRedis.addIngressRule(
      sgBastion,
      ec2.Port.tcp(6379),
      "SecurityGroupRedisBastionIngress"
    );
    cdk.Tags.of(sgRedis).add("Name", `${this.account}-${this.region}-sg-redis`);
    ...
    sgMemcache.addIngressRule(
      sgBastion,
      ec2.Port.tcp(11211),
      "SecurityGroupMemcacheBastionIngress"
    );
    cdk.Tags.of(sgMemcache).add(
      "Name",
      `${this.account}-${this.region}-sg-memcache`
    );

    const sgKubeWorker = new ec2.SecurityGroup(
      this,
      "SecurityGroupKubeWorker",
      {
        vpc,
        allowAllOutbound: false,
      }
    ...
    sgPostgress.addIngressRule(
      sgKubeWorker,
      ec2.Port.tcp(5432),
      "SecurityGroupPostgresKubeWorkerIngress"
    );
    sgRedis.addIngressRule(
      sgKubeWorker,
      ec2.Port.tcp(6379),
      "SecurityGroupRedisKubeWorkerIngress"
    );
    sgMemcache.addIngressRule(
      sgKubeWorker,
      ec2.Port.tcp(11211),
      "SecurityGroupMemcacheKubeWorkerIngress"
    );
    ...
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(8080),
      "SecurityGroupDeisElbHttpEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(6443),
      "SecurityGroupDeisElbHttpsEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(6443),
    ...
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(9090),
      "SecurityGroupDeisElbHealthcheckEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(2222),
      "SecurityGroupDeisElbGitEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(30080),
      "SecurityGroupDeisElbNginxHttpEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(30443),
      "SecurityGroupDeisElbNginxHttpsEgress"
    );
    sgDeisElb.addEgressRule(
      sgKubeWorker,
      ec2.Port.tcp(30254),
      "SecurityGroupDeisElbNginxHealthEgress"
    );

    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(8080),
      "SecurityGroupDeisElbHttpIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(6443),
      "SecurityGroupDeisElbHttpsIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(6443),
    ...
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(9090),
      "SecurityGroupDeisElbHealthcheckIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(2222),
      "SecurityGroupDeisElbGitIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(30080),
      "SecurityGroupDeisElbNginxHttpIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(30443),
      "SecurityGroupDeisElbNginxHttpsIngress"
    );
    sgKubeWorker.addIngressRule(
      sgDeisElb,
      ec2.Port.tcp(30254),
      "SecurityGroupDeisElbNginxHealthIngress"
    );

    cdk.Tags.of(sgDeisElb).add(
      "Name",
      `${this.account}-${this.region}-sg-deisElb`
    );

    const sgKubeCluster = new ec2.SecurityGroup(
      this,
      "SecurityGroupKubeCluster",
      {
        vpc,
        allowAllOutbound: false,
      }
    );
    sgKubeCluster.addIngressRule(
      sgBastion,
      ec2.Port.tcpRange(0, 65535),
      "SecurityGroupKubeClusterBastionIngress"
    );

    cdk.Tags.of(sgKubeCluster).add(
      "Name",
      `${this.account}-${this.region}-sg-kube-cluster`
    );

    const sgElasticSearch = new ec2.SecurityGroup(
      this,
    ...
    sgElasticSearch.addIngressRule(
      ec2.Peer.ipv4("10.0.0.0/16"),
      ec2.Port.tcp(443)
    );
    cdk.Tags.of(sgElasticSearch).add(
      "Name",
      `${this.account}-${this.region}-sg-elasticsearch`
    );
    ...
    vpc.publicSubnets.forEach((publicSubnet, i) => {
      new cdk.CfnOutput(
        this,
        `PublicSubnet${i + 1}${String.fromCharCode(97 + i)}RouteTable`,
        {
      ...
      new cdk.CfnOutput(
        this,
        `PublicSubnet${i + 1}${String.fromCharCode(97 + i)}`,
        {
    ...
    vpc.privateSubnets.forEach((privateSubnet, i) => {
      new cdk.CfnOutput(
        this,
        `PrivateSubnet${i + 1}${String.fromCharCode(97 + i)}`,
        {
